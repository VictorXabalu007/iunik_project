...
const listConsults = async (req, res) => {
  ...
  try {
    if (id < 1) {
      ...
      let consultores = [];
      if (mapStatus[status]) {
        consultores = await knex("usuarios")
          .select("*")
          .where("cargo_id", 4)
          .where("status", mapStatus[status] ?? "Ativo");
      } else {
      ...
      const consultoresComFaturamento = consultores.map((consultor) => ({
        ...consultor,
        faturamentoAgregado: faturamentoPorConsultor[consultor.id]
          ? faturamentoPorConsultor[consultor.id].toFixed(2)
          : "0.00",
      }));

      // Ordenar consultores por faturamento (do maior para o menor)
      consultoresComFaturamento.sort(
        (a, b) =>
          parseFloat(b.faturamentoAgregado) - parseFloat(a.faturamentoAgregado)
      );
      // Adicionar posição no ranking
      const consultoresComRanking = consultoresComFaturamento.map(
        (consultor, index) => ({
          ...consultor,
          position: index + 1,
          pedidosCount: pedidosCount[consultor.id] || 0,
        })
      );
      ...
      const position =
        faturamentoOutrosConsultores.filter(
          (c) => c.faturamento > faturamentoTotal
        ).length + 1;
